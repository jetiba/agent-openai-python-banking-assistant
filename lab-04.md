...
From the previous labs, the following files will be modified to add OpenTelemetry instrumentation and an Application Insights dashboard:
...
Now that we've reviewed the code and infrastructure changes, let's deploy everything. The new OpenTelemetry SDK will start sending telemetry to Application Insights automatically once the containers are running.
...
Before we can explore the monitoring dashboards, we need some telemetry data. Let's send a few requests to the API to generate traces, metrics, and logs in Application Insights.
...
### Performance
Under **Investigate → Performance**, explore request durations, dependency call times, and identify the slowest operations across your services.

### Sessions
Under **Usage → Sessions**, check how user sessions are tracked — you'll see session counts and durations based on the incoming traffic.

### Events
Under **Usage → Events**, explore the custom events and telemetry events being generated by your application.
...
This query retrieves all application log messages from the last hour. The `traces` table contains logs emitted by your Python code (via the `logging` module), which are automatically forwarded to Application Insights by the OpenTelemetry SDK. The `cloud_RoleName` column tells you which service emitted each log entry.
...
This query gives you a performance overview per service. It calculates the average and 95th percentile response times, total request count, and number of failures — grouped by `cloud_RoleName` so you can compare how each API is performing.
...
This query helps you identify any requests that took longer than 500ms to respond. These slow requests could indicate performance bottlenecks, cold starts, or resource contention worth investigating.
...
> **Note:** If no records are returned, that's a good sign — it means the application is responding quickly! To verify, remove the `| where duration > 500` line to see all requests and their actual response times.

### Dependency Calls Between Services

This query shows outgoing calls from one service to another (e.g., the Payment API calling the Transaction API). It helps you understand inter-service communication patterns and spot any slow or failing dependencies.
...
> **Tip:** If you want to go further, Application Insights also integrates with **Grafana**. You can explore the built-in Grafana dashboards provided by Application Insights for a richer visualization experience.
...
To try this out, go to the Azure Portal → your **Container App (account)** → **Console**, select the running container, and open a **Python** shell. Then run the following commands:
...
## Dig deeper — try to answer these questions:

- How long does a request to `/api/accounts` take to respond on average?
- How many `404` errors did you get in the last hour?
- What browser version was the active user using in the last hour?
