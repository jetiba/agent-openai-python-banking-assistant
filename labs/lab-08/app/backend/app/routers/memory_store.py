"""In-memory store for Lab 8 — with attachment support.

Extends the Lab 7 MemoryStore with simple dict-based attachment persistence
so the two-phase ChatKit upload protocol works without an external database.
"""
...
class MemoryStore(Store[dict[str, Any]]):
    ...
    Compared to Lab 7, the attachment methods are now implemented using a
    plain dictionary so that ``AttachmentMetadataStore`` can persist
    attachment metadata during the two-phase upload flow.
    """
    ...
    # -- Attachments (in-memory dict — NEW in Lab 8) ---------------------
