...
from agent_framework import SupportsAgentRun,FunctionTool,tool,Content,AgentResponseUpdate, Agent,InMemoryCheckpointStorage,WorkflowCheckpoint, WorkflowEvent
...
class HandoffOrchestrator:
    ...
    async def _resume_workflow_with_response(self, checkpoint_id: str, user_message: str) -> AsyncIterable[WorkflowEvent]:
        ...
        """
        events = self.workflow.run(checkpoint_id=checkpoint_id, checkpoint_storage=HandoffOrchestrator.checkpoint_storage, stream=True) #type: ignore
        ...
        for event in consumed_events:
            if event.type == "request_info":
                if isinstance(event.data, HandoffAgentUserRequest):
                        ...
                        return self.workflow.run(responses=responses, stream=True) #type: ignore
                else:
